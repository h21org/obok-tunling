...
iran_server_configuration() {  
    ...
    local transport=""
    while [[ ! "$transport" =~ ^(tcp|tcpmux|utcpmux|ws|wsmux|uwsmux)$ ]]; do
        echo -ne "[*] Transport type (tcp/tcpmux/utcpmux/ws/wsmux/uwsmux): "
        read -r transport

        if [[ ! "$transport" =~ ^(tcp|tcpmux|utcpmux|ws|wsmux|uwsmux)$ ]]; then
	...
	if [[ "$transport" == "tcp" ]]; then
	    while [[ "$accept_udp" != "true" && "$accept_udp" != "false" ]]; do
	        echo -ne "[*] Accept UDP connections over TCP transport (true/false): "
	        read -r accept_udp
    ...
    while [[ "$sniffer" != "true" && "$sniffer" != "false" ]]; do
        echo -ne "[*] Enable Sniffer (true/false): "
        read -r sniffer
	...
	local web_port=""
	while true; do
	    echo -ne "[*] Enter Web Port (0 to disable): "
	    read -r web_port
	    if [[ "$web_port" == "0" ]]; then
	        break
...
display_menu() {
    ...
    display_server_info
    display_backhaul_core_status
    check_core_version "http://backhaul.rocektserver.com/version"
    check_script_version "http://backhaul.rocektserver.com/script_version"
    echo
